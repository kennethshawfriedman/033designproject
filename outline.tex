...
\begin{document}
	...
	\IEEEtitleabstractindextext{%
		...
		\section{Server}
		The majority of the computational complexity is handled by a central server, connected via reliable wired connection to all APs. The server aggregates network data and handles client reassignment. This section details the components of the central server and their interactions with the rest of the system.
		
		\subsection{Network State Object}
		The server maintains the state of traffic in network in a hash table indexed by AP MAC addresses. The values of the hash table are pointers to objects that are also the nodes of an undirected network graph, which share links when their corresponding APs are within approximately 500 feet of each other in the real world. These links must be updated manually when APs are added, removed, or relocated. The nodes store static properties about the AP (such as its precise location), updated numbers on its network load (generated and successful transmission bits) from the network status updates that the APs send the sever, and a list of pointers to nodes of APs within a 500-foot radius.
