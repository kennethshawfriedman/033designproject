...
\begin{document}
	...
	\IEEEtitleabstractindextext{%
		...
		In addition to client assignment routing, the server logs a snapshot of network traffic on each AP every second for IS\&T records. Although each AP only updates the server every 30 seconds to keep traffic on the server minimized, a second-by-second snapshot of every AP captures the state, momentum, and load of the whole network with reasonable resolution.
