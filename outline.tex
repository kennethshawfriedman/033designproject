...
\twocolumn[
	...
	\markboth{6.033: Computer Systems Engineering, MIT, Spring 2016}{}
		...
		When a client connects to an AP, the AP forwards the client's initial connection message to the server, as detailed in sections 5.1 and 5.3. At this point, both the AP and the client assume that the client will remain connected to the AP unless explicitly reassigned by the server - a response is not needed or expected. This both reduces the number of messages and preserves a moderately functional network in event of a server failure. Furthermore, this reinforces our overall goal of simplicity by specifying a least-effort default behavior.
...
\section{Evaluation}
 -		One of the most critical aspects of system design is evaluation. We will analyze the system and determine how well it achieves the design goals of simplicity, scalability, and performance. Additionally, we will evaluate the system against several metrics including communications overhead, various latencies, network utilization, and user happiness.
		...
		The amount of time needed for a client to connect to an AP is a critical metric to the success of the system. If all clients are optimally connected to APs, but that optimal connection takes a long period of time, users will not be happy.\\
		\\
